...
class CategoriesController extends AnswersAppController {
	...
	var $uses = array('Answers.Answerscategory');
	...
	function index() {
		$this->Answerscategory->recursive = 0;
		$conditions = array('Answerscategory.parent_id !='=>0);
		$this->set('categories', $this->paginate(null,$conditions));
	}
	...
	function admin_add() {
		if (!empty($this->data)) {
			$this->Answerscategory->create();
			if ($this->Answerscategory->save($this->data)) {
				$this->Session->setFlash(__('The Category has been saved', true));
				$this->redirect(array('action'=>'index'));
			} else {
				$this->Session->setFlash(__('The Category could not be saved. Please, try again.', true));
		...
		$parents = $this->Answerscategory->generateTreeList(null, null, null, '&nbsp;&nbsp;&nbsp;');
